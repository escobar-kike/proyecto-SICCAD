<x-app-layout>
    <x-slot name="header">
        <h3 class="text-base215 font-bold text-xl text-gray-800 dark:text-gray-200 leading-tight">
    ...
    <div class="relative overflow-x-auto m-8">
        <table class="w-80% text-sm text-left text-gray-900 dark:text-gray-900">
            <thead
                class="text-sm text-base/15 font-bold text-gray-900 uppercase bg-gray-300 dark:bg-gray-900 dark:text-gray-900">
                <tr>
                    <th scope="col" class="px-6 text-base/15 font-bold py-3">
                        descripcion
                    </th>
                    
                    <th scope="col" class="px-6 text-base/15 font-bold py-3 text-center">
                        Accion
                    </th>
                    @foreach ($dictamenes as $dictamenes)
            <tbody>
                <tr class="bg-white border-2 text-center dark:bg-gray-800 dark:border-gray-900">
                    <th scope="row"
                        class="px-6 text-base/15 font-bold py-4 text-left text-gray-900 whitespace-nowrap dark:text-white">
                        {{ $dictamenes->descripcion }}
                    </th>

                    <td class="px-6 py-4 text-center">
                        <form action="{{ route('dictamenes.destroy', $dictamenes) }}" method="POST">

                            <a href="{{ route('dictamenes.edit', $dictamenes) }}"
                                class="btn btn-primary btn-sm px-2 py-2 text-sm font-medium text-center text-white bg-green-600 rounded-lg hover:bg-green-800 focus:ring-4 
                                focus:outline-none focus:ring-blue-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Editar</a>
