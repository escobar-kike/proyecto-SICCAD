<x-app-layout>
    <x-slot name="header">
        <h5 class="text-base/15 font-bold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Listado De Tipos De Hechos') }}
        </h5>
    </x-slot>
    @csrf
    ...
    <div class="m-8 w-full">
        <a href="{{ url('tipohechos/create') }}"
            class="btn btn-primary btn-sm px-2 py-2 text-sm font-medium 
        text-center text-white bg-blue-600 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none 
    ...
    <div class="relative overflow-x-auto m-8">
        <table class="w-80% text-sm text-center text-gray-900 dark:text-gray-900">
            <thead
                class="text-sm text-base/20 font-bold text-gray-900 uppercase bg-gray-300 dark:bg-gray-900 dark:text-gray-900">
                <tr>
                    <th scope="col" class="px-6 text-base/15 font-bold py-4">
                        descripcion
                    </th>
                    
                    <th scope="col" class="px-6 text-base/15 font-bold text-center py-4">
                        Accion
                    </th>
                    @foreach ($tipos_de_hechos as $tipos_de_hechos)
            <tbody>
                <tr class="bg-white border-2 text-left dark:bg-gray-800 dark:border-gray-700">
                    <th scope="row"
                        class="px-6 text-base/15 font-bold py-4 text-gray-900 whitespace-nowrap dark:text-white">
                        {{ $tipos_de_hechos->descripcion }}
                    </th>
                    

                    <td class="px-6 py-4">
                        <form action="{{ route('tipohechos.destroy', $tipos_de_hechos) }}" method="POST">

                            <a href="{{ route('tipohechos.edit', $tipos_de_hechos->id) }}"
                                class="btn btn-succes btn-sm px-2 py-2 text-sm font-medium text-center text-white bg-green-600 rounded-lg
